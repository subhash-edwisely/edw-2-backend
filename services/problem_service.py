...
def fetch_daily_challenge(user_id: int):
    ...
    - Slightly harder than user's average difficulty
    - Prefer topics the user solves most
    """

    # ----------------------------------
    # 1️⃣ Get solved problem IDs
    # ----------------------------------
    solved_problem_ids = [
        pid for (pid,) in
    ...
    # ----------------------------------
    # 2️⃣ Get user's top 3 solved topics
    # ----------------------------------
    favorite_topic_ids = [
        tid for (tid, _) in (
            db.session.query(
                Tag.id,
                func.count(SolvedProblem.id).label("cnt")
            )
    ...
    if solved_difficulties:
        avg = sum(diff_weight[d] * c for d, c in solved_difficulties) / sum(
            c for _, c in solved_difficulties
        )
        target_num = min(3, round(avg + 0.5))  # slightly harder
    else:
        target_num = 1  # Easy for new users
    ...
    target_difficulty = num_to_diff[target_num]
    ...
    if favorite_topic_ids:
        query = query.join(Problem.tags).filter(
            ProblemTag.tag_id.in_(favorite_topic_ids)
        )
    ...
    # ----------------------------------
    # 6️⃣ Return frontend-ready payload
    # ----------------------------------
    return {
        "id": selected.id,
        ...
        "tags": [
            {
                "id": pt.tag.id,
                "name": pt.tag.name,
                "category": pt.tag.category.value,
            }
            for pt in selected.tags
            if pt.tag
        ],
    }
